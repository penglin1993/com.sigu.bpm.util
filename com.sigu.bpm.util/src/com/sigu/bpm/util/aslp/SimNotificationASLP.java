 ...
 * SIM密信通消息提醒的aslp接口
 */
public class SimNotificationASLP implements ASLP {

	public SimNotificationASLP() {
	}
	...
	public ResponseObject call(Map<String, Object> params) {
		...
		try {
			if (type.equals("task")) {
				...
				if (task != null) {
					// 待办任务提醒
					SimUtil.sendTaskNotification(task);
				} else {
				...
				if (UtilString.isNotEmpty(receiver)) {
					// 普通消息提醒
					SimUtil.sendMsgNotification(receiver, content);
				} else {
