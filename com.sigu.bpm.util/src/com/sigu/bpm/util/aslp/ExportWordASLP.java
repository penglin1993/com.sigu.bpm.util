 ...
 * word文件按模板导出的aslp接口
 */
public class ExportWordASLP implements ASLP {

	public ExportWordASLP() {
	}
	...
	public ResponseObject call(Map<String, Object> params) {
		...
		try {
			...
			ResponseObject result = ExportUtil.exportWord(uc, templateName, templatePath, fileName, mapData);
			if (result.isOk()) {
				// 获取下载地址
	...
	private Map<String, Object> getMapData(String data) {
		Map mapData = JSONObject.parseObject(data, Map.class);
		return mapData;
	}
